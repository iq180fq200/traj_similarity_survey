...
object DualIndexingSolution {
  ...
  def main(args: Array[String]): Unit = {
    val sparkConf = new SparkConf().setAppName("RRSolution").set("spark.serializer", "org.apache.spark.serializer.KryoSerializer")
      //.set("spark.locality.wait", "0")
      .set("spark.driver.maxResultSize", "40g")//.setMaster("local[*]")
...
//    }
    ...
    val query_traj_filename = args(0)//"src/main/scala/resources/dft_query.txt"
    val traj_data_filename = args(1)//"src/main/scala/resources/dft_data.txt"
...
//    }.toArray.groupBy(_._1).map(x => x._2.map(_._2))
    ...
    var ee=System.currentTimeMillis();
    println("dual index time:"+(ee-ss))
    ...
    println("local index time:"+(ee-ss))
    ...
    ee=System.currentTimeMillis();
    println("global index time:"+(ee-ss))

    val end1 = System.currentTimeMillis()
    println("------------------------------------------------------------")
    println("Time to build indexes: " + (end1 - start1))
    println("------------------------------------------------------------")
    ...
    c_values.foreach(c => {
      ...
      queries.foreach(query_traj => {
        ...
        println("------------------------------------------------------------")
        println("Time to calculate pruning bound: " + (end2 - start2) )
        ...
        println("Time to calculate all saved traj_ids: " + (end3 - start3) )
        ...
        tot_time += (end4 - start2)
        println("Time to finish the final filter: " + (end4 - start4) )
        println("Time to knn search: " + (end4 - start2))
