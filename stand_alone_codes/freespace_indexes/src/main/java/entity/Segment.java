...
public class Segment {
    ...
    public double getDistance(Segment segment){
        double d1=this.pointToSegmentDistance(segment.s);
        double d2=this.pointToSegmentDistance(segment.e);
        double d3=segment.pointToSegmentDistance(this.s);
        double d4=segment.pointToSegmentDistance(this.e);
        return Math.max(Math.max(d1,d2),Math.max(d3,d4));
    }

    public static void main(String[] args){
        Segment s = new Segment(new Point(0.0,0.0),new Point(2.0,2.0));
        Segment o = new Segment(new Point(0.0,2.0),new Point(1.0,2.0));
        Point a = s.getIntersect(o);
